...
The `Protect branches (one click)` workflow now computes a diff before applying. By default it **refuses to apply** if the change would remove required checks.

- Set workflow input `refuse_on_removals=false` only if you intentionally want to remove required checks.

## Recommended allowlist_regex

The dry-run report (`artifacts/branch-protection-diff.md`) includes a generated **recommended allowlist_regex** you can paste into the workflow input to keep auto-detected checks stable.
