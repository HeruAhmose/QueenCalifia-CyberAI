...
_RUNTIME_SQLITE_PATHS = {
    "data/qc_os.db": Path("data/qc_os.db"),
    "./data/qc_os.db": Path("data/qc_os.db"),
    "qc_os.db": Path("data/qc_os.db"),
    "data/queen.db": Path("data/queen.db"),
    "./data/queen.db": Path("data/queen.db"),
    "queen.db": Path("data/queen.db"),
    "/var/data/qc_os.db": Path("/var/data/qc_os.db"),
    "/var/data/queen.db": Path("/var/data/queen.db"),
}
_TEST_SQLITE_PATHS: dict[str, Path] = {}


def _resolve_sqlite_path(db_path: Path | str) -> Path:
    """Map logical SQLite identifiers to trusted, non-tainted paths only."""
    ...
    if test_id:
        # Caller input is used only as an opaque dictionary key. The actual path
        # is generated by tempfile and a constant filename, so tainted input can
        # never become a filesystem path component.
        key = f"{test_id}\x00{raw}"
        path = _TEST_SQLITE_PATHS.get(key)
        if path is None:
            path = Path(tempfile.mkdtemp(prefix="qc-primary-db-")) / "primary.db"
            _TEST_SQLITE_PATHS[key] = path
        return path

    configured = (os.getenv("QC_DB_PATH") or "").strip()
    logical = configured if configured else raw
    logical = logical.replace("\\", "/")
    path = _RUNTIME_SQLITE_PATHS.get(logical)
    if path is None:
        raise ValueError(
            "SQLite runtime path is not approved; use data/qc_os.db, data/queen.db, "
            "/var/data/qc_os.db, /var/data/queen.db, or PostgreSQL via QC_DATABASE_URL"
        )
    return path
