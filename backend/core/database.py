"""QC OS — Database (merged: 12 tables, SHA-256 provenance, market history)"""
...
def audit(db_path: Path, event_type: str, actor: str, target: str | None = None, detail: dict | None = None) -> None:
    with get_db(db_path) as c:
        c.execute("INSERT INTO audit_log (event_type,actor,target,detail_json,created_at) VALUES (?,?,?,?,?)",
...
def log_event(db_path: Path, category: str, kind: str, subject: str, payload: Any) -> None:
    with get_db(db_path) as c:
        c.execute("INSERT INTO telemetry_events (category,kind,subject,payload_json,created_at) VALUES (?,?,?,?,?)",
                  (category, kind, subject, json_dumps(payload), utc_now()))

def save_market_snapshot(db_path: Path, asset_type: str, symbol: str, source: str, price: float | None, quote_ccy: str | None, payload: dict) -> None:
    with get_db(db_path) as c:
        c.execute("INSERT INTO market_snapshots (asset_type,symbol,source,price,quote_ccy,payload_json,created_at) VALUES (?,?,?,?,?,?,?)",
...
def get_market_history(db_path: Path, asset_type: str, symbol: str, limit: int = 24) -> list[dict]:
    with get_db(db_path) as c:
        rows = c.execute("SELECT price,quote_ccy,source,created_at FROM market_snapshots WHERE asset_type=? AND symbol=? ORDER BY id DESC LIMIT ?",
                         (asset_type, symbol, limit)).fetchall()
