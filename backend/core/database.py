...
POSTGRES_SCHEMA = SQLITE_SCHEMA.replace("INTEGER PRIMARY KEY AUTOINCREMENT", "BIGSERIAL PRIMARY KEY").replace("mission_id INTEGER NOT NULL REFERENCES", "mission_id BIGINT NOT NULL REFERENCES")
TRUSTED_SOURCES = (
...
def init_db(db_path: Path | str) -> None:
    with get_db(db_path) as c:
        ...
        for source_id, name, base_url, source_type, confidence in TRUSTED_SOURCES:
            c.execute("INSERT OR IGNORE INTO trusted_sources (id,name,base_url,source_type,confidence_score,enabled,created_at) VALUES (?,?,?,?,?,1,?)", (source_id, name, base_url, source_type, confidence, now))


def audit(db_path: Path | str, event_type: str, actor: str, target: str | None = None, detail: dict | None = None) -> None:
    with get_db(db_path) as c:
        c.execute("INSERT INTO audit_log (event_type,actor,target,detail_json,created_at) VALUES (?,?,?,?,?)", (event_type, actor, target, json_dumps(detail) if detail else None, utc_now()))


def log_event(db_path: Path | str, category: str, kind: str, subject: str, payload: Any) -> None:
    with get_db(db_path) as c:
        c.execute("INSERT INTO telemetry_events (category,kind,subject,payload_json,created_at) VALUES (?,?,?,?,?)", (category, kind, subject, json_dumps(payload), utc_now()))
...
def save_market_snapshot(db_path: Path | str, asset_type: str, symbol: str, source: str, price: float | None, quote_ccy: str | None, payload: dict) -> None:
    with get_db(db_path) as c:
        c.execute("INSERT INTO market_snapshots (asset_type,symbol,source,price,quote_ccy,payload_json,created_at) VALUES (?,?,?,?,?,?,?)", (asset_type, symbol, source, price, quote_ccy, json_dumps(payload), utc_now()))


def get_market_history(db_path: Path | str, asset_type: str, symbol: str, limit: int = 24) -> list[dict]:
    with get_db(db_path) as c:
        rows = c.execute("SELECT price,quote_ccy,source,created_at FROM market_snapshots WHERE asset_type=? AND symbol=? ORDER BY id DESC LIMIT ?", (asset_type, symbol, limit)).fetchall()
