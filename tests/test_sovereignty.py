...
class TestSovereigntyExecutor:
    ...
    def executor(self, audit_log):
        return SovereigntyExecutor(
            ...
            idempotency_check_fn=lambda r: False,  # v3.4: required for containment
        )
    ...
    def _make_crypto_approval(self, decision, executor_obj):
        """Helper: create a real Ed25519 dual-signed approval for testing."""
        from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
        from sovereignty.approvals import (
            InMemoryApprovalStore, KeyRegistry, PublicKeyRecord,
            sign_approval_ed25519, _b64e,
        )
        from sovereignty.schemas import SignatureAlg

        store = InMemoryApprovalStore()
        priv_a, priv_b = Ed25519PrivateKey.generate(), Ed25519PrivateKey.generate()
        registry = KeyRegistry([
            PublicKeyRecord(key_id="k-a", alg=SignatureAlg.ed25519, owner_id="admin_a",
                            public_key_b64=_b64e(priv_a.public_key().public_bytes_raw())),
            PublicKeyRecord(key_id="k-b", alg=SignatureAlg.ed25519, owner_id="admin_b",
                            public_key_b64=_b64e(priv_b.public_key().public_bytes_raw())),
        ])
        executor_obj.approval_store = store
        executor_obj.key_registry = registry
        dhash = stable_hash(decision.model_dump())
        rec = store.create(tenant_id="T-TEST", decision_hash=dhash, requested_by="requester")
        store.add_signature(rec.approval_id, sign_approval_ed25519(
            private_key=priv_a, key_id="k-a", approver_id="admin_a",
            decision_hash=dhash, nonce=rec.nonce))
        store.add_signature(rec.approval_id, sign_approval_ed25519(
            private_key=priv_b, key_id="k-b", approver_id="admin_b",
            decision_hash=dhash, nonce=rec.nonce))
    ...
    def test_containment_dry_run_default(self, executor, audit_log):
        decision = ModelDecision(
            action=ProposedAction.contain_host, confidence=0.95,
            summary="test", rationale="test rationale", targets=["10.0.0.1"],
        )
        aid = self._make_crypto_approval(decision, executor)
        req = ActionRequest(
            decision=decision, actor_id="exec-user", actor_role="admin",
            environment="prod", tenant_id="T-TEST", dry_run=True, approval_id=aid,
        )
        result = executor.execute(req)
    ...
    def test_containment_executes_with_approval(self, executor, audit_log, monkeypatch):
        ...
        req = ActionRequest(
            decision=decision, actor_id="exec-user", actor_role="admin",
            environment="prod", tenant_id="T-TEST", dry_run=False, approval_id=aid,
        )
        result = executor.execute(req)
        assert result["status"] == "executed"
    ...
    def test_audit_written_before_execution(self, executor, audit_log, monkeypatch):
        monkeypatch.setenv("QC_CONTAINMENT_MODE", "auto")
        decision = ModelDecision(
            action=ProposedAction.block_ip, confidence=0.95,
            summary="test", rationale="test rationale", targets=["10.0.0.1"],
        )
        aid = self._make_crypto_approval(decision, executor)
        req = ActionRequest(
