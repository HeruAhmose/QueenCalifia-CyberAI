...
count="$(docker run --rm postgres:18-alpine psql "$restore_url" -Atqc \
  "SELECT count(*) FROM information_schema.tables WHERE table_schema='public' AND table_type='BASE TABLE'")"
[[ "$count" == "0" ]] || { echo "restore verification target must have zero public base tables (found $count)" >&2; exit 3; }
