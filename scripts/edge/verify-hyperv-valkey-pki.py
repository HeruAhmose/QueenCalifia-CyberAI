...
def fail(message: str) -> None:
    raise SystemExit(f"HYPERV_VALKEY_PKI_EVIDENCE_ERROR={message}")
...
def verify_material() -> dict[str, Any]:
    virt = run(["systemd-detect-virt", "-v"]).strip().lower()
    if virt != "microsoft":
        fail(f"Hyper-V Valkey PKI evidence requires Microsoft virtualization; detected {virt or 'unknown'}")

    vm_id = hyperv_vm_id_raw()
    ...
    if git_status:
        fail("canonical repository checkout must be clean for Hyper-V PKI evidence")
    ...
    return {
        ...
        "hyperv_guest_vm_id_raw": vm_id,
        "guest_host_identity_fingerprint_sha256": host_identity_fingerprint(),
        "git_head": git_head,
...
def main() -> int:
    ...
    if AUTH_MARKER.exists():
        fail("authorization marker exists; preauthorization Hyper-V PKI evidence refused")
    ...
    evidence.update(
        {
            "schema": "queen-califia-hyperv-final-valkey-pki-evidence-v1",
    ...
    output = EVIDENCE_ROOT / f"hyperv-final-valkey-pki-{stamp}.json"
    if output.exists():
    ...
    print("HYPERV_FINAL_VALKEY_PKI_MATERIAL=PASS")
    print("HYPERV_FINAL_VALKEY_PKI_GENERATED=ELIGIBLE_FOR_HUMAN_REVIEW")
    print("HYPERV_FINAL_VALKEY_AUTHORITY_VERIFIED=NO")
    print(f"HYPERV_FINAL_VALKEY_PKI_EVIDENCE={output}")
    print(f"HYPERV_FINAL_VALKEY_PKI_EVIDENCE_SHA256={digest}")
    print(f"HYPERV_GUEST_VM_ID_RAW={evidence['hyperv_guest_vm_id_raw']}")
    print("HYPERV_FINAL_VALKEY_LEDGER_UPDATED=NO")
    print("HYPERV_FINAL_VALKEY_AUTHORIZATION_UPDATED=NO")
    return 0
