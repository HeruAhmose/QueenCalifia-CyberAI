...
if [[ "$ACTION" == "prepare" ]]; then
  [[ "$(docker inspect -f '{{.State.Health.Status}}' queen-califia-valkey 2>/dev/null || true)" == "healthy" ]] || { echo "Valkey must be healthy before preparing reboot proof" >&2; exit 3; }
...
[[ -f "$CHALLENGE" ]] || { echo "missing reboot challenge; run prepare before reboot" >&2; exit 3; }
"${compose[@]}" up -d
for _ in $(seq 1 45); do
  [[ "$(docker inspect -f '{{.State.Health.Status}}' queen-califia-valkey 2>/dev/null || true)" == "healthy" ]] && break
  sleep 2
done
[[ "$(docker inspect -f '{{.State.Health.Status}}' queen-califia-valkey)" == "healthy" ]] || { echo "Valkey unhealthy after reboot" >&2; exit 4; }
...
[[ "$current_boot" != "$before_boot" ]] || { echo "boot ID did not change; this is not a real reboot proof" >&2; exit 5; }
...
[[ "$actual" == "$expected" ]] || { echo "Valkey AOF reboot persistence failed" >&2; exit 6; }

for _ in $(seq 1 45); do
  [[ "$(docker inspect -f '{{.State.Health.Status}}' queen-califia-api 2>/dev/null || true)" == "healthy" ]] && break
  sleep 2
done
[[ "$(docker inspect -f '{{.State.Health.Status}}' queen-califia-api)" == "healthy" ]] || { echo "API unhealthy after reboot" >&2; exit 7; }
[[ "$(docker inspect -f '{{.State.Running}}' queen-califia-cloudflared)" == "true" ]] || { echo "cloudflared not running after reboot" >&2; exit 8; }
