...
SOVEREIGNTY_TABLES = ("qc_approval_records", "qc_used_nonces", "qc_audit_chain")
INCIDENT_TABLES = ("qc_ir_incidents", "qc_ir_runtime_state")
REMEDIATION_TABLES = ("qc_remediation_plans", "qc_remediation_action_log")
EVOLUTION_TABLES = (
    "health_log", "learned_patterns", "evolutions", "scan_intelligence",
    "false_positives", "network_baselines", "processed_scan_learning",
)
THREAT_TABLES = (
    "threat_feeds", "threat_indicators", "threat_cves", "threat_actors",
    "threat_sync_log", "threat_scheduler_lease",
)
...
AUTHORITATIVE_TABLES = (
    set(PRIMARY_TABLES) | set(DYNAMIC_TABLES) | set(SOVEREIGNTY_TABLES)
    | set(INCIDENT_TABLES) | set(REMEDIATION_TABLES) | set(EVOLUTION_TABLES)
    | set(THREAT_TABLES)
)
...
CREATE TABLE IF NOT EXISTS qc_approval_records (
    approval_id TEXT PRIMARY KEY, approval_json TEXT NOT NULL, updated_at DOUBLE PRECISION NOT NULL
);
CREATE TABLE IF NOT EXISTS qc_used_nonces (
    nonce TEXT PRIMARY KEY, used_at DOUBLE PRECISION NOT NULL
);
CREATE TABLE IF NOT EXISTS qc_audit_chain (
    sequence BIGINT PRIMARY KEY, timestamp DOUBLE PRECISION NOT NULL,
    record_json TEXT NOT NULL, record_hash TEXT NOT NULL,
    prev_chain_hash TEXT NOT NULL, chain_hash TEXT NOT NULL, hash_alg TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS qc_ir_incidents (
    incident_id TEXT PRIMARY KEY, attack_chain_id TEXT,
    incident_json TEXT NOT NULL, updated_at TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS qc_remediation_plans (
    plan_id TEXT PRIMARY KEY, plan_json TEXT NOT NULL, updated_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS qc_remediation_action_log (
    log_id BIGSERIAL PRIMARY KEY, action_json TEXT NOT NULL, created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS health_log (
    id BIGSERIAL PRIMARY KEY, component_id TEXT NOT NULL, status TEXT NOT NULL,
    error TEXT, healed INTEGER DEFAULT 0, timestamp TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS learned_patterns (
    pattern_id TEXT PRIMARY KEY, learning_type TEXT NOT NULL, source_engine TEXT NOT NULL,
    pattern_json TEXT NOT NULL, confidence DOUBLE PRECISION DEFAULT 0,
    observations INTEGER DEFAULT 1, first_seen TEXT NOT NULL, last_seen TEXT NOT NULL,
    applied INTEGER DEFAULT 0
);
CREATE TABLE IF NOT EXISTS evolutions (
    evolution_id TEXT PRIMARY KEY, evolution_type TEXT NOT NULL, description TEXT NOT NULL,
    payload_json TEXT NOT NULL, source_patterns TEXT, created_at TEXT NOT NULL,
    applied INTEGER DEFAULT 0, success INTEGER, impact_score DOUBLE PRECISION DEFAULT 0
);
CREATE TABLE IF NOT EXISTS scan_intelligence (
    id BIGSERIAL PRIMARY KEY, host_ip TEXT NOT NULL, port INTEGER, service TEXT,
    version TEXT, finding_type TEXT, severity TEXT, scan_time TEXT NOT NULL,
    remediated INTEGER DEFAULT 0
);
CREATE TABLE IF NOT EXISTS false_positives (
    id BIGSERIAL PRIMARY KEY, rule_id TEXT NOT NULL, finding_hash TEXT NOT NULL,
    marked_by TEXT DEFAULT 'operator', timestamp TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS network_baselines (
    baseline_key TEXT PRIMARY KEY, baseline_json TEXT NOT NULL, updated_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS processed_scan_learning (
    scan_id TEXT PRIMARY KEY, learned_at TEXT NOT NULL, source TEXT,
    learning_json TEXT, evolution_json TEXT
);
...
CREATE TABLE IF NOT EXISTS threat_feeds (
    feed_id TEXT PRIMARY KEY, name TEXT NOT NULL, source_url TEXT NOT NULL,
    feed_format TEXT NOT NULL, status TEXT NOT NULL, update_interval_sec INTEGER NOT NULL,
    last_sync DOUBLE PRECISION NOT NULL DEFAULT 0, last_success DOUBLE PRECISION NOT NULL DEFAULT 0,
    error_count INTEGER NOT NULL DEFAULT 0, ioc_count INTEGER NOT NULL DEFAULT 0,
    confidence_weight DOUBLE PRECISION NOT NULL DEFAULT 1.0, tags_json TEXT NOT NULL,
    auth_required INTEGER NOT NULL DEFAULT 0, created_at DOUBLE PRECISION NOT NULL,
    parser_config_json TEXT NOT NULL, headers_json TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS threat_indicators (
    indicator_id TEXT PRIMARY KEY, value TEXT NOT NULL, indicator_type TEXT NOT NULL,
    confidence DOUBLE PRECISION NOT NULL, severity TEXT NOT NULL, sources_json TEXT NOT NULL,
    first_seen DOUBLE PRECISION NOT NULL, last_seen DOUBLE PRECISION NOT NULL,
    last_updated DOUBLE PRECISION NOT NULL, expires_at DOUBLE PRECISION NOT NULL DEFAULT 0,
    tags_json TEXT NOT NULL, mitre_techniques_json TEXT NOT NULL, threat_actor TEXT,
    campaign TEXT, context_json TEXT NOT NULL, active INTEGER NOT NULL DEFAULT 1,
    decay_rate DOUBLE PRECISION NOT NULL DEFAULT 0.01
);
CREATE TABLE IF NOT EXISTS threat_cves (
    cve_id TEXT PRIMARY KEY, description TEXT NOT NULL, cvss_score DOUBLE PRECISION NOT NULL DEFAULT 0,
    severity TEXT NOT NULL, affected_products_json TEXT NOT NULL,
    exploit_available INTEGER NOT NULL DEFAULT 0, in_the_wild INTEGER NOT NULL DEFAULT 0,
    patch_available INTEGER NOT NULL DEFAULT 0, first_published DOUBLE PRECISION NOT NULL,
    last_modified DOUBLE PRECISION NOT NULL, references_json TEXT NOT NULL,
    mitre_techniques_json TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS threat_actors (
    actor_id TEXT PRIMARY KEY, name TEXT NOT NULL, aliases_json TEXT NOT NULL,
    nation_state TEXT, motivation TEXT, sophistication TEXT,
    target_industries_json TEXT NOT NULL, target_regions_json TEXT NOT NULL,
    known_techniques_json TEXT NOT NULL, known_tools_json TEXT NOT NULL,
    campaigns_json TEXT NOT NULL, ioc_count INTEGER NOT NULL DEFAULT 0,
    last_activity DOUBLE PRECISION NOT NULL DEFAULT 0, confidence DOUBLE PRECISION NOT NULL DEFAULT 0
);
CREATE TABLE IF NOT EXISTS threat_sync_log (
    id BIGSERIAL PRIMARY KEY, feed_id TEXT NOT NULL, success INTEGER NOT NULL,
    iocs_ingested INTEGER NOT NULL DEFAULT 0, cves_ingested INTEGER NOT NULL DEFAULT 0,
    actors_ingested INTEGER NOT NULL DEFAULT 0, detail_json TEXT,
...
def _incident_row(row: Mapping[str, Any]) -> tuple[Any, ...]:
    ...
    if payload_id != incident_id:
        raise RuntimeError(f"qc_ir_incidents identity mismatch: row={incident_id!r}, json={payload_id!r}")
    return (incident_id, payload.get("attack_chain_id") or None, row["incident_json"], row["updated_at"])

def _build_specs() -> dict[str, TableSpec]:
    specs = {table: TableSpec((), ()) for table in set(PRIMARY_TABLES) | set(DYNAMIC_TABLES)}
    ...
    for table, (columns, keys) in explicit.items():
        specs[table] = TableSpec(columns, keys, _incident_row if table == "qc_ir_incidents" else _identity(columns))
    return specs

TABLE_SPECS = _build_specs()
SERIAL_COLUMNS = {table: "id" for table in PRIMARY_SERIAL_TABLES}
SERIAL_COLUMNS.update({
    "qc_remediation_action_log": "log_id", "health_log": "id",
    "scan_intelligence": "id", "false_positives": "id", "threat_sync_log": "id",
})
...
def _table_spec(conn: sqlite3.Connection, table: str) -> TableSpec:
    ...
    missing = sorted(required - set(_source_columns(conn, table)))
    if missing:
...
def _normalize(value: Any) -> Any:
    ...
    if isinstance(value, float): return {"__float__": repr(value)}
    return value
...
def _source_rows(conn: sqlite3.Connection, table: str, spec: TableSpec) -> list[tuple[Any, ...]]:
    ...
    rows = conn.execute(f"SELECT * FROM {_quote_identifier(table)} ORDER BY {key_sql}").fetchall()
...
def _validate_json_columns(conn: sqlite3.Connection, table: str) -> None:
    ...
    for column in JSON_COLUMNS.get(table, ()):
        ...
        for row in conn.execute(f'SELECT "{column}" FROM {_quote_identifier(table)} WHERE "{column}" IS NOT NULL').fetchall():
            if row[0] in (None, ""): continue
            try: json.loads(row[0])
            except Exception as exc: raise RuntimeError(f"invalid JSON in {table}.{column}: {exc}") from exc
...
def _validate_semantic_invariants(conn: sqlite3.Connection, tables: set[str]) -> None:
    if "qc_approval_records" in tables:
        for row in conn.execute("SELECT approval_id, approval_json FROM qc_approval_records").fetchall():
            ...
            if record.approval_id != row["approval_id"]:
                raise RuntimeError(f"qc_approval_records identity mismatch: row={row['approval_id']!r}, json={record.approval_id!r}")
    ...
    if "qc_audit_chain" in tables:
        rows = conn.execute("SELECT sequence, record_json, record_hash, prev_chain_hash, chain_hash, hash_alg FROM qc_audit_chain ORDER BY sequence ASC").fetchall()
        prev = GENESIS_HASH
        for expected_sequence, row in enumerate(rows):
            sequence = int(row["sequence"])
            if sequence != expected_sequence:
                raise RuntimeError(f"qc_audit_chain sequence discontinuity: expected={expected_sequence}, actual={sequence}")
            record = json.loads(row["record_json"]); alg = row["hash_alg"]
            expected_record_hash = compute_record_hash(record, alg)
            if row["record_hash"] != expected_record_hash: raise RuntimeError(f"qc_audit_chain record hash mismatch at sequence {sequence}")
            if row["prev_chain_hash"] != prev: raise RuntimeError(f"qc_audit_chain predecessor mismatch at sequence {sequence}")
            expected_chain_hash = compute_chain_hash(prev, expected_record_hash, alg)
            if row["chain_hash"] != expected_chain_hash: raise RuntimeError(f"qc_audit_chain chain hash mismatch at sequence {sequence}")
            prev = row["chain_hash"]
...
def _dedupe_sources(sources: Sequence[SourceSpec]) -> list[SourceSpec]:
    ...
    for source in sources:
        by_path.setdefault(source.path.expanduser().resolve(), []).append(source.role)
...
def _inventory_sources(sources: Sequence[SourceSpec]) -> tuple[dict[str, tuple[SourceSpec, sqlite3.Connection, TableSpec]], list[dict[str, Any]], list[str]]:
    ...
    source_meta: list[dict[str, Any]] = []; blockers: list[str] = []
    for source in _dedupe_sources(sources):
        conn = _connect_source(source.path); tables = _source_tables(conn)
        unknown = sorted(tables - KNOWN_SOURCE_TABLES)
        if unknown:
            conn.close(); raise RuntimeError(f"{source.path} contains unrecognized application tables; update migration inventory before cutover: " + ", ".join(unknown))
        ...
        for table in unresolved:
            ...
            blockers.append(f"{UNRESOLVED_WRITER_TABLES[table]} remains a SQLite runtime writer ({source.path}:{table}, rows={count})")
        authoritative = sorted(tables & AUTHORITATIVE_TABLES); _validate_semantic_invariants(conn, set(authoritative))
        for table in authoritative:
            _validate_json_columns(conn, table)
            if table in ownership:
                prior = ownership[table][0]; conn.close()
                raise RuntimeError(f"authoritative table {table} appears in multiple SQLite sources: {prior.path} and {source.path}; refusing authority merge")
            ownership[table] = (source, conn, _table_spec(conn, table))
        source_meta.append({"role": source.role, "path": str(source.path), "sha256": _sha256_file(source.path), "tables": authoritative, "unresolved_writer_tables": unresolved})
    return ownership, source_meta, blockers

def _ensure_target_schema(target, source_tables: set[str]) -> None:
    for statement in _schema_statements(POSTGRES_SCHEMA): target.execute(statement)
    for table, ddl in DYNAMIC_TABLES.items():
        if table in source_tables: target.execute(ddl)
...
def _topology_cutover_blockers(topology_path: Path | None = None) -> list[str]:
    if topology_path is not None and topology_path.expanduser().resolve() != RUNTIME_TOPOLOGY_PATH.resolve():
        raise RuntimeError("runtime topology path is fixed to the repository-owned control-plane file")
    topology = json.loads(RUNTIME_TOPOLOGY_PATH.read_text(encoding="utf-8")); blockers: list[str] = []
    if topology.get("multi_replica_api_permitted") is not True: blockers.append("runtime topology does not permit multi-replica API")
    for connector in topology.get("sqlite_connectors", []):
        if connector.get("migration_required") and connector.get("production_cutover_complete") is not True:
            blockers.append(f"{connector.get('store', connector.get('path', 'sqlite-store'))} production cutover incomplete")
    for item in topology.get("api_file_state", []):
        if item.get("production_cutover_complete") is not True:
            blockers.append(f"file state {item.get('environment', item.get('path', 'unknown'))} not externalized/validated")
    return blockers

def migrate_runtime_state(*, sources: Sequence[SourceSpec], database_url: str, topology_path: Path | None = None, require_cutover_ready: bool = False) -> dict[str, Any]:
    if not database_url.startswith(("postgresql://", "postgres://")): raise RuntimeError("database_url must be a postgresql:// or postgres:// URL")
    if not sources: raise RuntimeError("at least one SQLite source is required")
    ownership: dict[str, tuple[SourceSpec, sqlite3.Connection, TableSpec]] = {}; target = None
    try:
        ...
        if require_cutover_ready and blockers: raise RuntimeError("Cutover is not ready; unresolved runtime-state blockers:\n- " + "\n- ".join(blockers))
        target = _connect_postgres(database_url); table_manifest = {}; source_rows_by_table = {}
        for table, (_, conn, spec) in ownership.items():
            rows = _source_rows(conn, table, spec); source_rows_by_table[table] = rows
            table_manifest[table] = {"rows": len(rows), "sha256": _digest_rows(rows), "columns": list(spec.columns), "key_columns": list(spec.key_columns)}
        with target.transaction():
            _ensure_target_schema(target, set(ownership)); _assert_target_empty(target, ownership)
            for table in sorted(ownership): _copy_table(target, table, ownership[table][2], source_rows_by_table[table])
            for table, column in SERIAL_COLUMNS.items():
                if table in ownership: _reset_sequence(target, table, column)
            for table in sorted(ownership):
                spec = ownership[table][2]; target_rows = _target_rows(target, table, spec); expected = table_manifest[table]
                if len(target_rows) != expected["rows"]: raise RuntimeError(f"verification failed for {table}: source={expected['rows']}, target={len(target_rows)}")
                actual_digest = _digest_rows(target_rows)
                if actual_digest != expected["sha256"]: raise RuntimeError(f"content verification failed for {table}: source_sha256={expected['sha256']}, target_sha256={actual_digest}")
        return {"version": 1, "kind": "queen-califia-runtime-state-migration", "created_at": datetime.now(timezone.utc).isoformat(), "sources": source_meta, "tables": table_manifest, "cutover_ready": not blockers, "cutover_blockers": blockers, "topology_path": str(RUNTIME_TOPOLOGY_PATH) if topology_path is not None else None}
    finally:
        seen_connections: set[int] = set()
        for _, conn, _ in ownership.values():
            if id(conn) not in seen_connections: seen_connections.add(id(conn)); conn.close()
...
def verify_postgres_manifest(database_url: str, manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        for table, expected in sorted(manifest.get("tables", {}).items()):
            if table not in AUTHORITATIVE_TABLES: raise RuntimeError(f"manifest contains unrecognized table: {table}")
            spec = TableSpec(tuple(expected["columns"]), tuple(expected["key_columns"])); rows = _target_rows(target, table, spec)
            actual = {"rows": len(rows), "sha256": _digest_rows(rows)}
            if actual["rows"] != int(expected["rows"]): raise RuntimeError(f"restore verification failed for {table}: expected rows={expected['rows']}, actual={actual['rows']}")
            if actual["sha256"] != expected["sha256"]: raise RuntimeError(f"restore verification failed for {table}: expected sha256={expected['sha256']}, actual={actual['sha256']}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--database-url", default=(os.getenv("QC_DATABASE_URL") or os.getenv("DATABASE_URL") or ""), help="PostgreSQL URL; defaults to QC_DATABASE_URL or DATABASE_URL")
    parser.add_argument("--require-cutover-ready", action="store_true", help="Refuse before migration unless topology and source inventory have no unresolved cutover blockers")
    parser.add_argument("--emit-manifest-json", action="store_true", help="Emit only the verified migration manifest as JSON on stdout for shell redirection or secure capture")
    parser.add_argument("--verify-manifest-stdin", action="store_true", help="Read a migration manifest as JSON from stdin and verify the PostgreSQL target/restore")
...
def _sources_from_args(args: argparse.Namespace) -> list[SourceSpec]:
    ...
    threat = args.threat_intel_sqlite or _default_path("QC_THREAT_INTEL_DB", "qc_threat_intel.db")
    sources = [
        SourceSpec("primary+incident+remediation", primary), SourceSpec("evolution", evolution),
        SourceSpec("threat-intelligence", threat), SourceSpec("audit-chain", args.audit_chain_sqlite or primary),
        SourceSpec("approvals", args.approvals_sqlite or primary),
    ]
    if args.live_scanner_sqlite is not None: sources.append(SourceSpec("live-scanner", args.live_scanner_sqlite))
    return sources

def main() -> int:
    args = parse_args()
    if args.verify_manifest_stdin:
        result = verify_postgres_manifest(args.database_url, json.load(sys.stdin))
        print("PostgreSQL runtime-state restore verified against migration manifest.")
        for table, info in result["tables"].items(): print(f"  {table}: rows={info['rows']} sha256={info['sha256']}")
        return 0
    manifest = migrate_runtime_state(sources=_sources_from_args(args), database_url=args.database_url, topology_path=RUNTIME_TOPOLOGY_PATH, require_cutover_ready=args.require_cutover_ready)
    ...
    for table, info in sorted(manifest["tables"].items()): print(f"  {table}: rows={info['rows']} sha256={info['sha256']}")
    if manifest["cutover_blockers"]:
        print("CUTOVER NOT READY. Remaining blockers:")
        for blocker in manifest["cutover_blockers"]: print(f"  - {blocker}")
    else: print("No migration/topology blockers detected by this tool.")
