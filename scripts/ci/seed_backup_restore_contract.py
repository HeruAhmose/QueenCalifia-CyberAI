...
CI-only helper for the pg_dump -> pg_restore contract. Paths are generated by
Python tempfile; no user-controlled filesystem path is accepted.
"""
...
def _clean_target(url: str) -> None:
    ...
    # The backup/restore contract uses a dedicated fresh CI database in the
    # workflow. Refuse accidental cleanup of anything outside the expected CI DB.
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="qc-backup-contract-") as tmp:
        root = Path(tmp)
        requested_primary = root / "queen.db"
